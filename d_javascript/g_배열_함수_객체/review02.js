...
// cf) 추가 기능: 장바구니에 이미 존재하느 상품이면 수량 업데이트
...
//? 1) 장바구니에 상품 추가
function addToCart(id, name, prime, quantity) {
...
function displayCart() {
  ...
  cart.forEach(item => {
    console.log(`${item.name} - Price: $${item.price}, Quantity: ${cart.quantity}`);
...
// function calculateTotal() {
//   let total = cart.reuduce((sum, iem) => sum + (item.price * item.quantity), 0);
// }
