...
class Book {
  ...
  constructor (id, title, author) {
    // 생성자 함수에서 this 키워드로 속성 정의되는 값은 필드로 정의
    this.id = id,
    this.title = title,
    this.author = author;
    this.isAvalilabe = true;
  }
...
class Library {
  ...
  displayBooks() {
    ...
    this.books.forEach(book => {
      console.log(
        `${book.id}: ${book.title} by ${book.author} - ${book.isAvalilabe ? '대여 가능' : '대여 중'}`
      );
    })
  }
  ...
  updateBook(id, newTitle, newAuthor) {
    ...
    book.newAuthor = newAuthor || book.author;
  ...
  filterBooksByTitle(keyword) {
    ...
    filtered.forEach(book => {
      console.log(`${book.id}: ${book.title} by ${book.author} - ${book.isAvalilabe ? '대여 가능' : '대여 중'}`);
    });
    return filtered;
  }

  // [필터링] 대여 가능 여부로 도서 필터링
  filterBooksByAvailable(isAvalilabe) {
    //? isAvailable 값에 따라 true면 출력 시 ${대여 가능}인 책 목록
    // false라면 출력 시 ${대여 중}인 책 목록
    const status = isAvalilabe ? '대여 가능' : '대여 중';
    const filtered = this.books.filter(book => book.isAvalilabe === isAvalilabe);
  ...
  // [추가 기능] 대여 가능 도서 수 집계
  countAvailableBooks() {
    //? iSAvilable이 true인 데이터만 뽑아 해당 배열의 길이를 측정

    // const count = this.books.filter(book => book.isAvalilabe === true)
    const count = this.books.filter(book => book.isAvalilabe).length;
...
// === 이승아의 책 목록 ===
// 1: SQLD 공부는 재밌어 by 이승아 - 대여 가능
// 5: 스프링 공부는 재밌어 by 이승아 - 대여 가능

busanLibrary.filterBooksByTitle('재밌어');
// === 제목에 재밌어가 포함된 책 목록 ===
// 1: SQLD 공부는 재밌어 by 이승아 - 대여 가능
// 4: 리액트 공부는 재밌어 by 이지훈 - 대여 가능
// 5: 스프링 공부는 재밌어 by 이승아 - 대여 가능
