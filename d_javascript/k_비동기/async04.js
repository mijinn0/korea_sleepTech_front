...
async function fetchUserData() {
  ...
  let data = await fetch('https://api.example.com/data'); // 시간 걸리는 작업 + await(기다림)

  let jsonData = await data.json(); // 가져온 json 데이터를 JS 객체로 변환: 시간 걸리는 작업 + await(기다림) *await로 담아낸 속성을 사용할 때도 awiat를 사용하는 게 좋음*
...
async function fetchCustomerData() {
  try {
    ...
    // try 블록에서 에러 발생 시 해당 에러를 매개 면수로 받아 처리
...
// 3) statusText
// : 응답의 HTTP 상태코드에 대한 텍트스 메시지 반환 (예. "OK", "NOT FOUND", "SERVER ERROR" 등)
