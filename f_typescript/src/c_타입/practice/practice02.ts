...
async function fetchUserData(userId: number) {
  ...
  try {
    ...
    // A instanceof B 연산자
    // : A데이터가 B 안에 포함되어 있는지 여부를 확인
    handleResponse({ error: e instanceof Error ? e.message : 'Unknown Error'});
  }
}

fetchUserData(1); // Name: Leanne Graham, Email: Sincere@april.biz
fetchUserData(2); // Name: Ervin Howell, Email: Shanna@melissa.tv
fetchUserData(22); // 네트워크 응답이 실패하였습니다.
// fetchUserData('안녕');