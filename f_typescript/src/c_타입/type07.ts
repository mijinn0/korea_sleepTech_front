...
console.log(arrowFunc('류민석')); 
console.log(arrowFunc('이상혁'));
...
function select(name?: string, nickname: string = '케리아') {
  if (name) {
...
select(); // 케리아 is guest
select('류민석'); // 케리아 is 류민석
select('프로게이머'); // 케리아 is 프로게이머*****
// cf) 선택적 매개변수와 기본 매개변수를 혼합 사용 시
// : 선택적 매개변수는 반드시!! 필수 매개변수(인자로 전달하는 값) 뒤에 작성!!
// - 기본 매개변수는 필수, 선택적 매개변수 양쪽에 어디든 작성 가능
// - 기본 매개변수가 앞서는 겨우 생략하려면 반드시 undefined 값을 전달

select(undefined, '프로게이머'); // 프로게이머 is guest
select('이상혁', '페이커'); // 페이커 is 이상혁
